 ...
 * (BE 스펙 확정): `GET /groups/:id/members`는 ACTIVE 멤버만 반환하므로 role=editor
 * (구 teacher — CHMO-604)가 1명이고 그 1명이 나면 마지막 선생님이다.
 *
...
export function isLastActiveTeacher(members: GroupMember[], myUserId: ID): boolean {
  const teachers = members.filter((m) => m.role === 'editor')
