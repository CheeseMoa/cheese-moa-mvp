 ...
 * 둘러보기(00-T)는 자동 노출 폐지(CHMO-565 — NN/g 실험(선행 튜토리얼은 과업 성공률을 못 올리고
 * 체감 난이도만 악화)이 근거)를 거쳐 CHMO-578에서 삭제됐다.
 * 안내는 코치 힌트(아래)가 실제 화면의 낯선 지점에서 1회 맡는다.
 *
...
// 그 자리 1회 안내(CoachHint) — "이 계정이 이 힌트를 봤는가"를 힌트 id 단위로 남긴다.
// 기록 시점은 '뜰 때'다: 닫을 때 남기면 새로고침·이탈로 1회 보장이 깨진다(CHMO-504와 같은 결정).
