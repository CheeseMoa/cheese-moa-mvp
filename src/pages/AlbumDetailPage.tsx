 ...
 * 인물 앨범은 앨범명 옆 [✎ 앨범 설정] 하나로 모은다(AlbumSettingsSheet — 08 카드 이름 줄 탭과 같은 시트):
 ...
 * (이 앨범에만 있는 사진은 영구 삭제 경고) → DELETE /albums/:id → 08 복귀(replace). 사진 전량 삭제·이동으로 앨범이 비어도 앨범은 남는다(CHMO-418 —
...
export function AlbumDetailPage() {
  ...
  return (
    <PhoneShell>
      <Header
        backTo={eventPath}
        // 08이 헤더에 이벤트명을 띄우게 됐으니 여기 뒤로가기도 그 이름을 그대로 부른다
        // (이벤트 조회는 이동 확인용으로 이미 하고 있다 — 도착 전엔 중립 라벨로 폴백)
        backLabel={eventApi.data?.name ?? '이벤트'}
        backDisabled={busy}
        right={
          album && hasPhotos ? (
            <button
