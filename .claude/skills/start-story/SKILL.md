...
description: >-
  ...
  develop 기준 feature 브랜치 생성까지. 사용자가 "/start-story", "스토리 시작", "CHMO-### 작업 시작"으로 요청할 때 사용.
---
...
- GitHub 기준(base) 브랜치: `develop` (Git Flow — `docs/convention.md`)
...
6. **브랜치 생성**
   ...
   - `git switch develop && git pull --ff-only` (실패하면 중단하고 사용자에게 알림).
   - 요약을 짧은 영문 kebab-case slug로 변환(한글 요약이면 핵심을 영문으로). `git switch -c feature/CHMO-<번호>-<slug>`.
7. **요약 출력** — 스토리 제목·AC 체크리스트, 생성한 브랜치명, 설정한 SP, (담당자 수동 안내), 상태=진행 중, 다음 단계(구현 → `/test-guide`로 확인 → `/finish-story`). 이어서 해당 스토리 구현을 시작한다.

## 주의
- 브랜치명·커밋·PR에 항상 `CHMO-<번호>`를 포함해 Jira 개발 패널이 자동 연동되게 한다.
- `main`·`develop`에는 직접 커밋/푸시하지 않는다(가드 훅이 차단). 마무리는 `/finish-story`.
