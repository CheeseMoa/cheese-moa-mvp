...
- **API 계약 기준은 실 BE다**(오리진 `http://3.35.177.22`, 스웨거 `/v3/api-docs`) — `docs/api-spec.md`와 안 맞는 부분은 **FE를 BE에 맞춘다**. BE 규약: 성공/실패 모두 `{isSuccess, code, message, result}` 봉투 · 숫자 ID(int64) · 경로에 `/api/v1` 프리픽스 없음(vite 프록시가 rewrite) · 시각이 오프셋 없는 UTC(CHMO-205 전까지 FE 보정). 봉투·에러 코드·시각 차이는 `src/api/client.ts`·`src/api/errors.ts`가 흡수한다 — 화면 코드는 봉투를 모른다. 남은 차이(리소스 필드명 `groupId`≠`id`·`eventDate`·대문자 enum·AuthResponse에 user 객체 없음 등)의 적응은 CHMO-192(엔드포인트 계층)·195(목 이행)·196(문서 갱신) 몫.
...
- `api/` **공통 API 클라이언트**(CHMO-191) — `client.ts`(fetch 래퍼: 실 BE 봉투 `result` 언랩·시각 'Z' 보정 + MSW 평문 이중 흡수 + 뮤테이션 401 공용 `redirectIfUnauthorized`)·`errors.ts`(BE 에러 코드→FE 의미 코드 매핑, 미지 코드는 통과) · `lib/` `auth.ts`(제작자 토큰)·`viewer.ts`(뷰어 토큰)·`clipboard.ts`(복사 헬퍼)·`pin.ts`(4자리 PIN 규칙 단일 원천 — UI·목 공유) · `hooks/` `useApi.ts`·`useAlive.ts`(언마운트 후 늦은 응답 차단) · `types/api.ts`(API 계약 타입 — `ID = number`, 라우트 파라미터는 ID 값으로 쓸 때 `Number()` 변환).
