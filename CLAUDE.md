...
현재 단계: **스캐폴딩(CHMO-106) + 공용 컴포넌트 세트(CHMO-107) + MSW 목 레이어(CHMO-108·109) 완료.** Vite+React18+TS 토대·Tailwind v3 디자인 시스템·전 라우트 뼈대·화면 스텁 18개 + 재사용 UI 14종(`src/components/ui/`, 데모 `/dev/components`) + 목 DB·픽스처·api-spec 전 엔드포인트 핸들러(`src/mocks/` — 인증·모임·이벤트·앨범·검수·공개·뷰어). 화면별 실제 UI는 후속 스토리에서 구현. 소스 오브 트루스는 여전히 `docs/`.
...
- `/finish-story` — 커밋·푸시·PR 생성(테스트 가이드 포함) + Jira PR 코멘트. PR 머지 후 재실행 시 브랜치 정리 + Jira '완료'.
