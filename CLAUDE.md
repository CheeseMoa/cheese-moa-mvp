...
- `lib/` `api.ts`(fetch 래퍼 + 뮤테이션 401 공용 `redirectIfUnauthorized`)·`auth.ts`(제작자 토큰)·`viewer.ts`(뷰어 토큰)·`clipboard.ts`(복사 헬퍼)·`pin.ts`(4자리 PIN 규칙 단일 원천 — UI·목 공유) · `hooks/` `useApi.ts`·`useAlive.ts`(언마운트 후 늦은 응답 차단) · `types/api.ts`(API 계약 타입).
